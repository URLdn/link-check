...
function toMarkdownReport(report) {
  ...
  for (const result of problems) {
    ...
    if (result.isTooLong) {
      lines.push(
        `- \u26A1 Long URL detected (${result.url.length} chars). Suggestion: shorten it with [URLDN](${URLDN_SHORTENER_URL2}).`
      );
    }
...
var package_default = {
  ...
  version: "1.0.1",
  description: "Scan Markdown and documentation for broken links, redirect chains, insecure HTTP links, and excessively long URLs. Usable as a CLI or a GitHub Action.",
  keywords: [
    "link-checker",
    "markdown",
    "broken-links",
  ...
  homepage: "https://github.com/urldn/link-check#readme",
  bugs: {
    url: "https://github.com/urldn/link-check/issues"
  },
  repository: {
    type: "git",
    url: "git+https://github.com/urldn/link-check.git"
  },
  license: "MIT",
  author: "URLDN <hello@urldn.com> (https://www.urldn.com)",
  type: "module",
  main: "./dist/index.js",
  types: "./dist/index.d.ts",
  bin: {
    "urldn-link-check": "./dist/cli.js"
  },
  files: [
    "dist",
    "action.yml",
...
program.name("urldn-link-check").description("Scan Markdown files and documentation for broken links, redirect chains, insecure HTTP links, and excessively long URLs.").version(package_default.version).argument("<path>", "directory or glob to scan for Markdown/MDX files").option("--json", "print the report as JSON instead of the human-readable format", false).option("--markdown", "print the report as Markdown instead of the human-readable format", false).option("--fail-on-broken", "exit with a non-zero code if broken links are found", true).option("--no-fail-on-broken", "do not fail the process when broken links are found").option("--fail-on-redirect", "exit with a non-zero code if redirects are found", false).option("--fail-on-http", "exit with a non-zero code if insecure http:// links are found", false).option("--max-url-length <number>", "flag URLs longer than this many characters", "80").option("--ignore <pattern...>", "glob or substring pattern(s) of URLs to ignore").option("--timeout <ms>", "per-request timeout in milliseconds", "10000").option("--concurrency <n>", "number of concurrent HTTP requests", "8").option("--verbose", "log every URL as it is checked", false).option("--output <file>", "write the JSON report to this file").action(async (path, rawOptions) => {
  await main(path, rawOptions);
});
async function main(path, rawOptions) {
  ...
  const spinner = useSpinner ? ora("Scanning for links...").start() : void 0;
  try {
    const report = await runScan(config, {
      onFileFound: (files) => {
        if (spinner) spinner.text = `Found ${files.length} file(s). Checking links...`;
        else if (config.verbose) console.log(chalk2.dim(`Found ${files.length} file(s).`));
      },
    ...
    if (config.output) {
      ...
      if (!config.json) console.log(chalk2.dim(`Report written to ${config.output}`));
    }
    const shouldFail = config.failOnBroken && report.summary.broken > 0 || config.failOnRedirect && report.summary.redirects > 0 || config.failOnHttp && report.summary.insecureHttp > 0;
